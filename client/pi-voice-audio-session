...
old_player=$(cat "$active_player_file" 2>/dev/null || true)
old_player_command=$(tr '\0' ' ' <"/proc/$old_player/cmdline" 2>/dev/null || true)
if [[ $old_player =~ ^[0-9]+$ ]] && [[ $old_player_command == *mpv* && $old_player_command == *pi-voice-mpv-* ]] && kill -0 "$old_player" 2>/dev/null; then
