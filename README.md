...
- A configurable Wav2Vec2 CTC model aligns clean Kokoro audio in a separate worker. When voice mode is enabled, Kokoro and the aligner warm concurrently in the background and remain resident in RAM until Pi exits or reloads. Each audio segment may wait up to 750 ms for alignment. Later waits normally remain hidden behind preceding playback, while synthesis and alignment continue through the ahead-of-playback pipeline. If alignment is late or unavailable, duration-weighted word timing is used; if phone feedback is unavailable, the desktop playback clock is estimated.
