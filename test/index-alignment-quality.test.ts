...
test("worker quality reaches the widget independently of clock estimates and preserves paused framing", async t => {
	...
	assert.match(widget(), /^○ Idle · message 1\/1 · timing pending \[local\]\n/);
	assert.equal(host.widgetLines()?.at(-1), "Word timing: unknown/pending");
	worker.emit({ type: "loading" });
	await settle();
	assert.match(widget(), /^◷ Waiting · message 1\/1 · timing pending \[local\]\n/);
	worker.emit({ type: "speaking" });
	await settle();
	assert.match(widget(), /^▶ Playing · message 1\/1 · timing pending \[local\]\n/);
	await host.shortcut("f8");
	assert.match(widget(), /^⏯ Paused · message 1\/1 · timing pending \[local\]\n/);
	await host.shortcut("f8");
