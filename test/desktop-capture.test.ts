...
for (const source of ["failed", "empty", "pcm", "stopped"]) test(`desktop capture: ${source} recorder with real ffmpeg`, async t => {
	...
	if (source === "pcm" || source === "stopped") {
		...
		await assert.rejects(capture, /no decodable audio; check the selected device's recorder, microphone access, and audio tools/);
