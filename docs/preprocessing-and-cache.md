...
Missing descriptions use `editModel`; existing descriptions are content-addressed by prompt version, narration mode, context mode, language, and source code—not the selected LLM, thinking level, or its runtime system/tool prefix. Changing the generator only affects cache misses. With `codeDescriptionContext: "conversation"`, the key also includes a deterministic serialization of Pi's provider-compatible historical context through the closing fence. Future turns do not change that prefix; identical code in a different discussion gets a different description. With the default `block-only`, identical blocks share descriptions without including discussion context. Live narration, written callouts, timing preprocessing, and concurrent workers share in-flight requests and persisted results.
