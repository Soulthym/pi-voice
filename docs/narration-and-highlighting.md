...
After that initial placement, Voice normally leaves the viewport alone while the word remains in the 20–80% visible band (native end-follow is the exception below). Each time a new spoken word moves past the 80% mark, it is re-anchored at 20%. F6/F7/F9/F10/F11 replay and seek actions, F8 pause/resume, `/voice attention`, and `/voice scroll-to` all re-arm this behavior. The first marker lookup establishes the active message's transcript position; subsequent words reuse that anchor until layout or target changes invalidate it, rather than periodically rendering the whole transcript.
