...
`codeDescriptionContext` defaults to `block-only`, which sends no preceding discussion with a fence. For the best context-specific narration, explicitly choose `conversation`. Pi Voice then sends Pi's resolved, provider-compatible messages—including the applicable compaction summary—before the concerned fence. Depending on the retained conversation, those messages can contain user/assistant text, images, assistant tool calls and arguments, and tool results. The concerned block itself is sent exactly once.

With `editModel: "current"` (or a pin resolving to the same active model), the request preserves the normal model request as an exact prefix: Pi's effective system prompt, active tool schemas, and resolved messages through the user turn. It then appends the assistant output through the fence and a narration request. The same Pi session ID and provider-default cache retention allow supported providers to reuse a still-live normal-conversation prompt cache. Pi Voice does not create a separate local copy of that provider cache, and reuse is not guaranteed: it depends on provider support, cache lifetime, minimum prompt size, and exact model/prefix equality. A different pinned `editModel` receives the structured conversation but not Pi's system prompt or active tool schemas because there is no normal prompt cache to reuse.

Tool output is excluded from candidate-resolution context. In `conversation` mode, the stable description-cache identity covers the deterministic provider-compatible context through the closing fence, plus the effective system prompt and active tool schemas when the active model is reused; `block-only` identities contain no discussion context. Requests do not become conversation messages. Pi Voice checks each isolated request—including structured messages, system prompt, and tools—against the selected model's context window; if it cannot fit, it reports the condition and uses local structural narration rather than silently truncating context.
